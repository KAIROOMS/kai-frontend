...
const handleSubmit = (e) => {
  ...
  // Untuk Hybrid dan Online → minta link
  if (formData.jenisRapat === "Online" || formData.jenisRapat === "Hybrid") {
    setShowLinkPopup(true);
  } else {
    setShowBookingOptionPopup(true); // Offline → langsung pilih jenis
  }
};
        ...
        {realtimeStatus.map((m, i) => {
          ...
          return (
            <tr key={i}>
              <td
                onClick={() => {
                  console.log("🧩 ID dikirim:", m._id);
                  if (m.jenisRapat === "Online" && m.linkMeet) {
                    window.open(m.linkMeet, "_blank");
                  } else if (m.jenisRapat === "Hybrid") {
                    navigate('/hybrid-meeting-status', {
                      state: {
                        _id: m._id,
...
<button className="action-btn" onClick={() => navigate('/riwayat-rapat')}>
  ...
  onClick={async () => {
    try {
      ...
      body: JSON.stringify({
    ...formData, linkMeet: formData.linkMeet || ''
    })
})
  ...
  navigate('/hybrid-meeting-status', {
    state: {
      ...
      namaRapat: formData.namaRapat,
      tanggal: formData.tanggal,
      waktuMulai: formData.waktuMulai,
      waktuSelesai: formData.waktuSelesai,
      lokasi: formData.lokasi,
      ruangan: formData.ruangan,
      jenisRapat: formData.jenisRapat,
      kapasitas: formData.kapasitas,
      catatan: formData.catatan,
      linkMeet: formData.linkMeet || ''
    }
  });
} else {
  navigate('/room-status', {
    state: {
      ...
      console.error("❌ Error saat instant meeting:", err);
      alert("Gagal menyimpan data meeting.");
    }
  }}
>
